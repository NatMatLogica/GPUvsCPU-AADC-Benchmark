...
public:
    ...
    //  If randoms.bin exists, reads from file for consistency with Python GPU.
    //
    ...
    void generateRandoms(int num_randoms_per_path) {
        ...
        // Try to read from randoms.bin (generated by Python for consistency)
        std::ifstream randoms_file("randoms.bin", std::ios::binary);
        if (randoms_file.is_open()) {
            std::cout << "  Reading randoms from randoms.bin..." << std::endl;
            // File format: row-major doubles (num_paths x num_steps)
            for (int mc_i = 0; mc_i < m_mc_iterations; mc_i++) {
                std::vector<double> random_vec(num_randoms_per_path);
                randoms_file.read(reinterpret_cast<char*>(random_vec.data()),
                                  num_randoms_per_path * sizeof(double));
