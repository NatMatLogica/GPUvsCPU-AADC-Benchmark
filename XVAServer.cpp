...
int run_pricing(const int threads_num, const std::string input_file) {
    ...
    if (data_out.contains("AADC results")) {
        ...
        // If AADC CVA/DVA are 0, try to use GPU exposures (more accurate than AADC exposures)
        if (aadc_cva == 0.0 && aadc_dva == 0.0) {
            // First try GPU exposures file (generated by Python GPU backend)
            std::ifstream gpu_exp_file("gpu_exposures.bin", std::ios::binary);
            if (gpu_exp_file.is_open()) {
                // File format: 2 x num_pricing_times doubles (avg_pee, avg_nee)
                std::vector<double> pee(num_pricing_times);
                std::vector<double> nee(num_pricing_times);
                gpu_exp_file.read(reinterpret_cast<char*>(pee.data()), num_pricing_times * sizeof(double));
                gpu_exp_file.read(reinterpret_cast<char*>(nee.data()), num_pricing_times * sizeof(double));
                gpu_exp_file.close();

                auto [computed_cva, computed_dva] = compute_xva_from_exposures(pee, nee, data_in);
                aadc_cva = computed_cva;
                aadc_dva = computed_dva;
                std::cout << "  CVA/DVA computed from GPU exposures: CVA=" << aadc_cva
                          << ", DVA=" << aadc_dva << "\n";
            } else if (data_out["AADC results"].contains("PEE") &&
                       data_out["AADC results"].contains("NEE")) {
                // Fallback to AADC exposures (may be inaccurate)
